...
function guessMaxSeats(seats, tablename) {
  ...
  const num = seats.reduce((max, x) => Math.max(max, x.seatno), activeSeats)
  // normalize to 2-max, 6-max, 9-max or 10-max
  if (num >= 10) return 10
  if (num >= 9) return 9
  return 6
}
...
class IPokerPostProcessor {
  ...
  process() {
    ...
    const { actions: posts } = this._processActions(this._hand.postActions)
    const { actions: preflop, allin: pallin } =
      this._processActions(this._hand.preflopActions)
    const { actions: flop, allin: fallin } =
      this._processActions(this._hand.flopActions)
    const { actions: turn, allin: tallin } =
      this._processActions(this._hand.turnActions)
    const { actions: river, allin: rallin } =
      this._processActions(this._hand.riverActions)
    ...
    fixAllinActionsForAll({ preflop, flop, turn, river }, allins)
  ...
  */
  ...
  _processTable(seats, button) {
    ...
    // button is same as small blind in heads up
    // this is the best way to determine this as it's not indicated for sure anywhere else
    const maxseats = (this._sbPlayer != null && this._sbPlayer === this._buttonPlayer
      ? 2
      : guessMaxSeats(seats, tablename)
    )
  ...
  _processActions(actions) {
    ...
    const unsorted = actions
      .map(x => {
        ...
        if (this._sb == null && type === 'sb') {
          this._sb = amount
          this._sbPlayer = x.player
        }
        if (this._bb == null && type === 'bb') this._bb = amount

        const no =  parseInt(x.no)
        return {
            no
          , player: x.player
          , type
    ...
    // Mark allin actions, allin indicators are pseudo actions that affect
    // the last action of the player before it unless it has an amount which
    // denotes a call.
    sorted
      .forEach(x => {
        if (x.type === 'allin') {
          if (x.amount != null) {
            ...
            // mark last action of player as allin
            const lastAction = lastActions.get(x.player)
            if (lastAction != null) lastAction.allin = true
          }
          allin = true
        } else {
          lastActions.set(x.player, x)
        }
      })

    // Then remove those pseudo actions and pull out only the data we need
    const sortedActions = sorted
      .filter(x => x.type !== 'allin')
      .map(x => {
        const res = {
            player: x.player
          , type: x.type
        ...
        if (x.allin) res.allin = true
        return res
      })

    return { actions: sortedActions, allin }
  }
